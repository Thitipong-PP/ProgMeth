...
public class CPTSMachine {
	...
	public static boolean addStation(String name) {
		...
		totalStationNumber = totalStationNumber + 1;
		int id = getTotalStationNumber();

		stationlist.add(new Station(name, id));
	...
	public static boolean isStationExisted(String name) {
		/* FILL CODE */
		for (Station x:getStationlist()) if (x.getName() == name) return true;
		return false;
	}
	...
	public static boolean buyTicket(int type, Station start, Station end) {
		...
		else if (!isStationExisted(start.getName()) || !isStationExisted(end.getName())) isSuccess = false;
		else if (type < 0 || type > 2) isSuccess = false;
		else if (type == 2 && newTicket.getStationDistance(start, end) > 6) isSuccess = false;
		else if (newTicket.getStationDistance(start, end) == 0) return false;
		else isSuccess = true;

		if(isSuccess) {
			/* FILL CODE */
			ticketlist.add(newTicket);
			System.out.println("Bought "+ newTicket.getDescription() +", for "+ newTicket.calculatePrice() +" Baht!");
			return true;
		}else {
